...
const apiBaseUrl = isProductionBuild
  ? productionApiBaseUrl || genericApiBaseUrl || '/api'
  : devApiBaseUrl || genericApiBaseUrl || 'http://localhost:4000/api'

const networkErrorMessage = isProductionBuild
  ? 'Servidor de autenticação indisponível. Verifique se o backend publicado está ativo.'
  : 'Servidor de autenticação indisponível em DEV. Inicie o backend com "yarn --cwd docs/server dev" ou configure VITE_API_BASE_URL_DEV.'
